...
do
  ln -s /data/schwartzlab/eren/Chapter1/SISRS_Run/aligned_contigs/"${contig}".fasta /data/schwartzlab/eren/Chapter1/CONTIGS/Annotation/ALIGNED/ALL_50K/"${contig}".fasta
done


#cut -d'/' -f 1 alignment_locs_m5_nogap.txt | uniq -c | sort -bgr | sed 's/[[:space:]]//' > 0921_sorted_frequencies.csv
